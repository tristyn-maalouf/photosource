...
function getRelevantPhotos() {
  ...
  var searchString = document.querySelector('#search-string').value.toLowerCase();
  firebase.firestore().collection('photos').where('combinedTags', 'array-contains', searchString).get()
    .then(querySnapshot => {
      querySnapshot.forEach(documentSnapshot => {
        showPhoto(documentSnapshot.data().imageUrl);
      });
